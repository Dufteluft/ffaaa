...
function CreateLobby(playerId, settings)
    ...
    Utils.Print('Lobby erstellt: ' .. settings.name)
...
AddEventHandler('ffa:joinLobby', function(data)
    local lobbyId = data
    if type(data) == 'table' then lobbyId = data.lobbyId end
...
AddEventHandler('ffa:setTeam', function(data)
    local team = data
    if type(data) == 'table' then team = data.team end
    local state = PlayerStates[source]
    if state then
        state.team = team
        UpdateLobbyPlayers(state.lobbyId)
...
AddEventHandler('ffa:fetchLobbies', function(data)
    ...
    for id, lobby in pairs(Lobbies) do
        local match = false
        if tab == 'ffa' then if lobby.isPersistent then match = true end
        elseif tab == 'list' then if not lobby.isPersistent then match = true end
        end
...
MySQL.ready(function()
    ...
    for _, map in ipairs(Config.Maps) do
        ...
        Utils.Print('Persistent Lobby: ' .. map.label)
    end
end)
...
AddEventHandler('ffa:kickPlayer', function(data)
    local targetId = data
    if type(data) == 'table' then targetId = data.id end
    local state = PlayerStates[source]
    if state and state.lobbyId then
        local lobby = Lobbies[state.lobbyId]
