...
function CreateLobby(playerId, settings)
    ...
    Lobbies[lobbyId] = {
        ...
        mapLabel = map.label,
        mode = settings.mode,
        loadout = settings.loadouts, -- Jetzt ein Array von Keys
...
-- Event: Einstellungen updaten (Host)
...
AddEventHandler('ffa:updateSettings', function(settings)
    ...
    lobby.loadout = settings.loadouts
...
AddEventHandler('ffa:joinLobby', function(data)
    if JoinLobby(source, data.lobbyId) then
        TriggerClientEvent('ffa:lobbyJoined', source, Lobbies[data.lobbyId])
    end
end)
...
-- Aktualisiert die Spielerliste für alle in der Lobby
