...
function CreateLobby(playerId, settings)
    ...
    Lobbies[lobbyId] = {
        ...
        mapLabel = map.label,
        mode = settings.mode,
        loadout = settings.loadout, -- Now can be a table (multi-select)
    ...
    Utils.Print('Lobby erstellt: ' .. settings.name .. ' (' .. lobbyId .. ')')
...
function JoinLobby(playerId, lobbyId)
    ...
    if #lobby.players >= lobby.maxPlayers and lobby.status ~= 'playing' then return false end
...
AddEventHandler('ffa:fetchLobbies', function(data)
    ...
    for id, lobby in pairs(Lobbies) do
        local match = false
        if tab == 'ffa' and lobby.isPersistent then match = true
        elseif tab == 'lobby' and not lobby.isPersistent then match = true end
