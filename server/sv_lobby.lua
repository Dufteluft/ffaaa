...
function CreateLobby(playerId, settings)
    ...
    if playerId ~= -1 then
        ...
        -- Automatisch Timer für persistente Lobbys starten
        if settings.isPersistent and settings.roundTime > 0 then
...
AddEventHandler('ffa:updateSettings', function(settings)
    ...
    -- Synchronisiere Lobby-Daten an alle Spieler in der Lobby
    for _, pid in ipairs(lobby.players) do
