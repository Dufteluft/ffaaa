...
PlayerStates = {} -- Stores the state of each player (Lobby ID, Team, Kills, etc.)
...
function CreateLobby(playerId, settings)
    ...
    Lobbies[lobbyId] = {
        ...
        mapLabel = map.label,
        mode = settings.mode,
        loadout = settings.loadout, -- Now can be an array
    ...
    Utils.Print('Lobby created: ' .. settings.name .. ' by ' .. xPlayer.getName())
...
AddEventHandler('ffa:joinLobby', function(data)
    local lobbyId = data.lobbyId or data -- Handle both table and string
...
-- Update Lobby Players for all in lobby
...
-- Persistent Lobbies Initialization
MySQL.ready(function()
    ...
    for _, map in ipairs(Config.Maps) do
        ...
        Lobbies[lobbyId] = {
            ...
            mapLabel = map.label,
            mode = 'ffa',
            loadout = {'pistol', 'smg', 'assault', 'sniper', 'shotgun'},
            roundTime = 0,
            maxPlayers = 32,
        ...
        Utils.Print('Persistent FFA Lobby initialized: ' .. map.label)
    end
end)

-- Event: Quick Join (Tab 1)
