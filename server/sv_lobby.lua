...
function CreateLobby(playerId, settings)
    ...
    Lobbies[lobbyId] = {
        ...
        mapLabel = map.label,
        mode = settings.mode,
        loadout = settings.loadout, -- Kann Array oder String sein
...
function JoinLobby(playerId, lobbyId)
    local sLobbyId = tostring(lobbyId)
    local lobby = Lobbies[sLobbyId]
    ...
    SetPlayerRoutingBucket(playerId, tonumber(sLobbyId))
    UpdateLobbyPlayers(sLobbyId)
    return true
end
...
AddEventHandler('ffa:joinLobby', function(lobbyId)
    if JoinLobby(source, lobbyId) then
        ...
        -- Wenn persistente Lobby, direkt starten
...
AddEventHandler('ffa:kickPlayer', function(targetId)
    ...
    if state and state.lobbyId then
        ...
        if lobby and lobby.host == src then
            local targetPid = tonumber(targetId)
            if targetPid then
                LeaveLobby(targetPid)
                TriggerClientEvent('esx:showNotification', targetPid, _U('kicked_message'))
            end
        end
    end
end)
