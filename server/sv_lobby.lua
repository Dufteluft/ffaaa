...
AddEventHandler('ffa:createLobby', function(settings)
    ...
    if lobbyId then
        TriggerClientEvent('ffa:lobbyCreated', source, Lobbies[lobbyId], source)
    end
end)
...
AddEventHandler('ffa:joinLobby', function(data)
    ...
    if JoinLobby(source, lobbyId) then
        TriggerClientEvent('ffa:lobbyJoined', source, Lobbies[lobbyId], source)
    end
end)
...
-- Persistente Lobbys Initialisierung
