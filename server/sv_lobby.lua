...
function CreateLobby(playerId, settings)
    ...
    Lobbies[lobbyId] = {
        ...
        mapLabel = map.label,
        mode = settings.mode,
        loadout = settings.loadout, -- Kann ein Array sein
...
AddEventHandler('ffa:joinLobby', function(data)
    ...
    if JoinLobby(source, lobbyId) then
        ...
        -- Wenn es eine persistente Lobby ist, direkt ins Spiel starten
...
AddEventHandler('ffa:fetchLobbies', function(data)
    ...
    for id, lobby in pairs(Lobbies) do
        ...
        elseif tab == 'list' and not lobby.isPersistent then match = true end
