...
-- Eigene _U Funktion für den Server, da shared/sh_utils.lua eventuell nicht reicht
function _U(str, ...)
    local loc = Config.Locales[Config.Locale]
    if loc and loc[str] then
        return string.format(loc[str], ...)
    else
...
-- Funktion: Spielerliste synchronisieren
...
AddEventHandler('ffa:fetchLobbies', function(data)
    local list = {}
    local filterTab = (data and data.tab) and data.tab or 'ffa'
    local mapFilter = data and data.mapId or 'all'
    local playerFilter = data and data.playerFilter or 'all'
    ...
    for id, lobby in pairs(Lobbies) do
        ...
        -- Map Filter
        if isMatch and mapFilter ~= 'all' and lobby.mapId ~= mapFilter then
            isMatch = false
        end

        -- Player Filter (Not Full)
        if isMatch and playerFilter == 'not-full' and #lobby.players >= lobby.maxPlayers then
            isMatch = false
        end
