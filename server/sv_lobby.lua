...
function UpdateLobbyPlayers(lobbyId)
    ...
    for _, pid in ipairs(lobby.players) do
        ...
        table.insert(playersInfo, { id = pid, name = state.name, team = state.team, ready = state.ready, isHost = (pid == lobby.host) })
    end
    for _, pid in ipairs(lobby.players) do TriggerClientEvent('ffa:updateLobbyPlayers', pid, playersInfo) end
end
...
AddEventHandler('ffa:sendLobbyChat', function(data)
    ...
    if state and state.lobbyId then
        ...
        for _, pid in ipairs(lobby.players) do TriggerClientEvent('ffa:addChatMessage', pid, state.name, data.message) end
    end
end)
...
AddEventHandler('ffa:fetchLobbies', function(data)
    ...
    for id, lobby in pairs(Lobbies) do
        local isMatch = (filterTab == 'ffa' and lobby.isPersistent) or (filterTab == 'custom' and not lobby.isPersistent)
        if isMatch then
            table.insert(list, {
                id = id, name = lobby.name, hostName = lobby.hostName,
                playerCount = #lobby.players, maxPlayers = lobby.maxPlayers,
                mapLabel = lobby.mapLabel, mode = lobby.mode, status = (lobby.status == 'playing' and 'ACTIVE' or 'waiting')
            })
        end
    end
...
MySQL.ready(function()
    ...
    for _, map in ipairs(Config.Maps) do
        local lobbyId = GenerateLobbyId()
        Lobbies[lobbyId] = {
            id = lobbyId, name = "FFA " .. map.label, host = -1, hostName = "SYSTEM",
            isPersistent = true, mapId = map.id, mapLabel = map.label, mode = 'ffa', loadout = 'all',
            roundTime = 15, maxPlayers = 32, vehiclesAllowed = false, friendlyFire = false, respawnTime = 3, killLimit = 0,
...
AddEventHandler('ffa:kickPlayer', function(targetId)
    ...
    if state and state.lobbyId then
        local lobby = Lobbies[state.lobbyId]
        if lobby and lobby.host == source then LeaveLobby(tonumber(targetId)) end
    end
end)
