    ...
    if state and state.lobbyId then
        ...
        if lobby and lobby.host == source and not lobby.isPersistent then
            ...
            for _, pid in ipairs(playersToKick) do
                LeaveLobby(pid)
                TriggerClientEvent('esx:showNotification', pid, _U('btn_close_lobby'))
            end
        end
    end
end)
...
AddEventHandler('ffa:fetchLobbies', function(data)
    ...
    local mapId = data and data.mapId or 'all'
    local notFull = data and data.notFull or false
    ...
    for id, lobby in pairs(Lobbies) do
        ...
        if isMatch then
            if mapId ~= 'all' and lobby.mapId ~= mapId then isMatch = false end
            if notFull and #lobby.players >= lobby.maxPlayers then isMatch = false end
        end
