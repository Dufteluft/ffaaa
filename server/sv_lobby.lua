...
AddEventHandler('ffa:joinLobby', function(lobbyId)
    if JoinLobby(source, lobbyId) then
        ...
        -- Wenn persistent und bereits läuft, direkt starten
        if Lobbies[lobbyId].status == 'playing' then
            PlayerStates[source].team = 'ffa'
            TriggerClientEvent('ffa:gameStarting', source, Lobbies[lobbyId])
        end
    end
end)
...
AddEventHandler('ffa:updateSettings', function(data)
    local state = PlayerStates[source]
    if state and state.lobbyId then
        local lobby = Lobbies[state.lobbyId]
        if lobby and lobby.host == source then
            ...
            lobby.loadout = data.loadout
            lobby.roundTime = data.roundTime
            lobby.maxPlayers = data.maxPlayers
            lobby.vehiclesAllowed = data.vehiclesAllowed
            lobby.friendlyFire = data.friendlyFire
            lobby.respawnTime = data.respawnTime
            lobby.killLimit = data.killLimit
            lobby.timer = data.roundTime * 60
...
-- Initialisierung der vordefinierten Lobbys
MySQL.ready(function()
    ...
    for _, map in ipairs(Config.Maps) do
        ...
        -- Persistente Lobbys direkt auf spielend setzen
        for id, lobby in pairs(Lobbies) do
