...
function CreateLobby(playerId, settings)
    ...
    Lobbies[lobbyId] = {
        ...
        mapLabel = map.label,
        mode = settings.mode,
        loadout = settings.loadout, -- Kann Array sein
...
AddEventHandler('ffa:joinLobby', function(lobbyId)
    if JoinLobby(source, lobbyId) then
        ...
        TriggerClientEvent('esx:showNotification', source, '~r~Lobby ist voll oder existiert nicht.')
    end
end)
...
AddEventHandler('ffa:voteMap', function(mapId)
    ...
    if state and state.lobbyId then
        ...
        if lobby then
            ...
            -- Wir könnten hier Live-Votes an Clients senden
        end
    end
end)

-- Initialisierung persistente Lobbys
