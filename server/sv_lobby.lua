...
AddEventHandler('ffa:updateSettings', function(settings)
    ...
    if lobby and lobby.host == src and lobby.status == 'waiting' then
...
AddEventHandler('ffa:closeLobby', function()
    ...
    if lobby and lobby.host == src then
        local playersToKick = {}
        for _, pid in ipairs(lobby.players) do table.insert(playersToKick, pid) end
...
AddEventHandler('ffa:fetchLobbies', function(data)
    ...
    local filterMaps = data and data.filters and data.filters.map or 'all'
    local filterPlayers = data and data.filters and data.filters.players or 'all'
    ...
    for id, lobby in pairs(Lobbies) do
        ...
        if isMatch then
            -- Additional Filters
            if filterMaps ~= 'all' and lobby.mapId ~= filterMaps then isMatch = false end
...
MySQL.ready(function()
    ...
    for _, map in ipairs(Config.Maps) do
        ...
        Utils.Print('Persistent Lobby init: ' .. map.label)
    end
end)
