...
function CreateLobby(playerId, settings)
    ...
    Utils.Print('Lobby erstellt: ' .. Lobbies[lobbyId].name .. ' von ' .. xPlayer.getName())
...
function JoinLobby(playerId, lobbyId)
    ...
    if #lobby.players >= lobby.maxPlayers then
        TriggerClientEvent('esx:showNotification', playerId, _U('lobby_full'))
        return false
    end
...
-- Event: Lobby Einstellungen aktualisieren (Host)
...
AddEventHandler('ffa:updateSettings', function(settings)
    ...
    if state and state.lobbyId then
        ...
        if lobby and lobby.host == source and lobby.status == 'waiting' then
            ...
            if settings.killLimit ~= nil then lobby.killLimit = settings.killLimit end

            for _, pid in ipairs(lobby.players) do
                TriggerClientEvent('ffa:lobbyJoined', pid, lobby) -- Sync Settings
                TriggerClientEvent('esx:showNotification', pid, _U('settings_updated'))
            end
        end
    end
end)
    ...
    if state and state.lobbyId then
        ...
        if lobby and lobby.host == source and not lobby.isPersistent then
            ...
            for _, pid in ipairs(players) do
                TriggerClientEvent('esx:showNotification', pid, _U('close_lobby'))
                LeaveLobby(pid)
            end
        end
    end
...
AddEventHandler('ffa:kickPlayer', function(data)
    ...
    if state and state.lobbyId then
        ...
        if lobby and lobby.host == source then
            ...
            TriggerClientEvent('esx:showNotification', tonumber(targetId), _U('player_kicked'))
        end
    end
end)
...
MySQL.ready(function()
    ...
    for _, map in ipairs(Config.Maps) do
        ...
        StartGameTimer(lobbyId) -- Start individual timers
    end
end)
