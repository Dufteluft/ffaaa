...
AddEventHandler('ffa:joinLobby', function(data)
    ...
    if JoinLobby(source, lobbyId) then
        ...
        TriggerClientEvent('esx:showNotification', source, 'Lobby konnte nicht beigetreten werden.')
    end
end)
...
function LeaveLobby(playerId)
    ...
    -- Status via Client wiederherstellen
...
AddEventHandler('ffa:closeLobby', function()
    ...
    if lobby and lobby.host == playerId and not lobby.isPersistent then
