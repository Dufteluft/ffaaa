...
AddEventHandler('ffa:voteMap', function(data)
    ...
    if state and state.lobbyId then
        ...
        if lobby and not lobby.isPersistent then
            -- Wir aktualisieren die Map der Lobby basierend auf dem Vote (vereinfacht: der erste Vote gewinnt für die nächste Runde)
            -- In einer produktiven Umgebung würde man Votes zählen und nach Timer die Map setzen.
            ...
            if map then
                ...
                for _, pid in ipairs(lobby.players) do
                    TriggerClientEvent('ffa:addChatMessage', pid, 'SYSTEM', 'Nächste Map wurde gewählt: ' .. map.label)
                end
            end
        end
    end
