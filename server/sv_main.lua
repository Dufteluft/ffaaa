...
function StartGameTimer(lobbyId)
    ...
    if lobby.roundTime == 0 then
        lobby.timer = 3600 -- 1 Stunde Default für persistente Lobbys falls kein Timer
    end
    ...
    Citizen.CreateThread(function()
        while Lobbies[lobbyId] and Lobbies[lobbyId].status == 'playing' do
            ...
            if not lobby.isPersistent or (lobby.isPersistent and lobby.roundTime > 0) then
...
function EndGame(lobbyId, reason)
    ...
    if lobby.isPersistent then
        lobby.timer = (lobby.roundTime > 0 and lobby.roundTime * 60 or 3600)
        lobby.status = 'playing'
        lobby.scoreBlue = 0
        lobby.scoreRed = 0
        StartGameTimer(lobbyId)
