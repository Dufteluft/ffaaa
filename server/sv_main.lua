...
function StartGameTimer(lobbyId)
    ...
    if lobby.roundTime == 0 then
        lobby.timer = 3600 -- Default 1 hour for persistent
    end
    ...
    if lobby.isPersistent then
        Citizen.CreateThread(function()
            ...
            if Lobbies[lobbyId] then
                ...
                Lobbies[lobbyId].timer = 3600
                StartGameTimer(lobbyId)
            end
        end)
    else
        -- Reset für Custom Lobbys
...
AddEventHandler('ffa:voteMap', function(mapId)
    ...
    if state and state.lobbyId then
        ...
        if lobby then
            ...
            for _, pid in ipairs(lobby.players) do
                TriggerClientEvent('ffa:addChatMessage', pid, _U('system_msg'), 'Die Map wurde auf ' .. lobby.mapLabel .. ' geändert.')
            end
        end
    end
end)
