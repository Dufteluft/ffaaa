...
function EndGame(lobbyId, reason)
    ...
    if lobby.isPersistent then
        Citizen.CreateThread(function()
            Citizen.Wait(15000) -- 15 Sekunden Winner Screen
...
AddEventHandler('ffa:voteMap', function(data)
    ...
    if state and state.lobbyId then
        local lobby = Lobbies[state.lobbyId]
        if lobby then
            -- In einer echten Implementierung würden wir Stimmen zählen
            -- Hier ändern wir die Map einfach direkt für die nächste Runde
            lobby.mapId = mapId
            local map = Utils.GetMapById(mapId)
            if map then lobby.mapLabel = map.label end

            for _, pid in ipairs(lobby.players) do
                TriggerClientEvent('ffa:addChatMessage', pid, 'SYSTEM', 'Nächste Map: ' .. lobby.mapLabel)
            end
        end
    end
end)
