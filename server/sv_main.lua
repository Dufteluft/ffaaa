    ...
    if lobby.isPersistent then
        Citizen.CreateThread(function()
            ...
            if Lobbies[lobbyId] then
                RestartPersistentLobby(lobbyId)
            end
        end)
    end
end

function RestartPersistentLobby(lobbyId)
    local lobby = Lobbies[lobbyId]
    lobby.status = 'playing'
    lobby.scoreBlue = 0
    lobby.scoreRed = 0
...
AddEventHandler('ffa:playerKilled', function(killerId)
    ...
    TriggerClientEvent('ffa:playSound', victim, 'death') -- Assuming you have a death sound or similar
...
AddEventHandler('ffa:voteMap', function(mapId)
    ...
    if state and state.lobbyId then
        ...
        if lobby and not lobby.isPersistent then
            ...
            for _, pid in ipairs(lobby.players) do
                TriggerClientEvent('ffa:addChatMessage', pid, 'SYSTEM', 'Map changed to ' .. lobby.mapLabel)
            end
        end
    end
end)
