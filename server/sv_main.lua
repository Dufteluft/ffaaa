...
function StartGameTimer(lobbyId)
    ...
    Citizen.CreateThread(function()
        while Lobbies[lobbyId] and Lobbies[lobbyId].status == 'playing' do
            ...
            if not l or l.status ~= 'playing' then break end
...
AddEventHandler('ffa:updateSettings', function(newSettings)
    local state = PlayerStates[source]
    if state and state.lobbyId then
        local lobby = Lobbies[state.lobbyId]
        if lobby and lobby.host == source and not lobby.isPersistent then
            ...
            -- Sync updated settings to all players in lobby
            for _, pid in ipairs(lobby.players) do
                TriggerClientEvent('ffa:lobbyJoined', pid, lobby) -- Reuse joined event to refresh UI
            end
        end
    end
end)
