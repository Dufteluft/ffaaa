...
function StartGameTimer(lobbyId)
    Citizen.CreateThread(function()
        while Lobbies[lobbyId] and Lobbies[lobbyId].status == 'playing' do
            ...
            if lobby.timer <= 0 then EndGame(lobbyId, 'Zeit abgelaufen'); break end
            for _, pid in ipairs(lobby.players) do
...
function EndGame(lobbyId, reason)
    ...
    lobby.status = 'ended'
    local winnerName, maxKills, winnerTeam = 'Niemand', -1, 'none'
    ...
    for _, pid in ipairs(lobby.players) do
        local ps = PlayerStates[pid]
        if ps then
            table.insert(stats, { name = ps.name, kills = ps.kills, deaths = ps.deaths, kd = string.format("%.2f", (ps.deaths > 0) and (ps.kills / ps.deaths) or (ps.kills + 0.0)) })
        end
    end
    ...
    for _, pid in ipairs(lobby.players) do
        ...
        if state then UpdatePlayerStats(pid, state.kills, state.deaths, (winnerName == state.name or (winnerTeam ~= 'none' and state.team == winnerTeam))) end
    ...
    if lobby.isPersistent then
        lobby.timer, lobby.status, lobby.scoreBlue, lobby.scoreRed = lobby.roundTime * 60, 'playing', 0, 0
        StartGameTimer(lobbyId)
    end
end
...
AddEventHandler('ffa:playerKilled', function(killerId)
    ...
    if killerId and killerId ~= -1 and PlayerStates[killerId] then TriggerClientEvent('ffa:updateHUDStats', killerId, PlayerStates[killerId].kills, PlayerStates[killerId].deaths) end
end)
...
AddEventHandler('ffa:voteMap', function(mapId)
    ...
    if state and state.lobbyId then
        ...
        if lobby and not lobby.isPersistent then
            ...
            for _, pid in ipairs(lobby.players) do TriggerClientEvent('ffa:addChatMessage', pid, 'SYSTEM', 'Die Map wurde auf ' .. lobby.mapLabel .. ' geändert.') end
        end
    end
end)
