...
AddEventHandler('ffa:startGame', function()
    ...
    if lobby and lobby.host == source and #lobby.players >= 1 then
        ...
        for _, pid in ipairs(lobby.players) do
            ...
            if pState and pState.team == 'blue' then blueCount = blueCount + 1
            elseif pState and pState.team == 'red' then redCount = redCount + 1 end
        end
...
AddEventHandler('ffa:playerKilled', function(killerId)
    ...
    local lobby = Lobbies[tostring(victimState.lobbyId)]
    if not lobby then return end
