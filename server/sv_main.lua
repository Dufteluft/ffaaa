...
AddEventHandler('ffa:startGame', function()
    ...
    -- Mindestens 2 Spieler erforderlich (hier 1 für Tests, laut Anforderung ab 2)
    if lobby and (lobby.host == source or lobby.isPersistent) and #lobby.players >= 2 then
...
function StartGameTimer(lobbyId)
    ...
    Citizen.CreateThread(function()
        while Lobbies[lobbyId] and Lobbies[lobbyId].status == 'playing' do
            ...
            if lobby.timer <= 0 and lobby.roundTime > 0 then
...
function EndGame(lobbyId, reason)
    ...
    if lobby.mode == 'tdm' then
        if lobby.scoreBlue > lobby.scoreRed then
            winnerName = _U('team_blue') .. ' ' .. _U('winner_suffix')
            winnerTeam = 'blue'
        elseif lobby.scoreRed > lobby.scoreBlue then
            winnerName = _U('team_red') .. ' ' .. _U('winner_suffix')
            winnerTeam = 'red'
        else
    ...
    -- Sieg-Logik für FFA
        ...
        for _, pid in ipairs(lobby.players) do
            ...
            if state and state.kills > maxKills then
                ...
                winnerName = state.name .. ' ' .. _U('winner_suffix')
            end
        end
    end
    ...
    for _, pid in ipairs(lobby.players) do
        ...
        local state = PlayerStates[pid]
        if state then
            local isWin = (winnerName:find(state.name)) or (winnerTeam ~= 'none' and state.team == winnerTeam)
