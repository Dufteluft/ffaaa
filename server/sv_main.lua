...
function StartGameTimer(lobbyId)
    ...
    Citizen.CreateThread(function()
        while Lobbies[lobbyId] and Lobbies[lobbyId].status == 'playing' do
            ...
            local mins, secs = math.floor(l.timer / 60), l.timer % 60
...
function EndGame(lobbyId, reason)
    ...
    -- Map Voting Logic
    Citizen.CreateThread(function()
        Citizen.Wait(15000) -- Wait for voting
        local votedMap = nil
        local maxVotes = -1
        local voteCounts = {}
        ...
        if votedMap then
            lobby.mapId = votedMap
            local map = Utils.GetMapById(votedMap)
...
AddEventHandler('ffa:closeWinnerScreen', function()
    ...
    if state and state.lobbyId then
        ...
        if lobby and not lobby.isPersistent then
            -- Just let the client handle UI transitions
        end
    end
end)
