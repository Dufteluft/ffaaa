...
-- Menü-Steuerung
RegisterNetEvent('ffa:toggleMenu')
AddEventHandler('ffa:toggleMenu', function()
    if isMenuOpen then
        isMenuOpen = false
...
AddEventHandler('ffa:settingsUpdated', function(lobby)
    currentLobby = lobby
    -- Wir senden 'lobbyJoined' um die UI zu aktualisieren ohne den Wartebereich neu zu öffnen
    SendNUIMessage({ action = 'lobbyJoined', lobby = lobby })
end)
