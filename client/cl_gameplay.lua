...
-- Spielstart-Vorbereitung (Teleportation, Loadout)
...
-- Teilt das gewählte Loadout an den Spieler aus
...
-- Grenzprüfung und Fahrzeug-Spawn
-- Grenzprüfung, Fahrzeug-Spawn und Waffen-Validierung
        ...
        if playerState.isInGame and currentLobby then
            ...
            if map then
                ...
                if dist > map.radius then
                    ESX.ShowNotification('~r~' .. _U('game_ended'))
                ...
                if currentWeapon ~= GetHashKey('WEAPON_UNARMED') then
                    ...
                    if type(loadoutKeys) == "table" then
                        for _, k in ipairs(loadoutKeys) do
                            if checkAllowed(k) then allowed = true break end
                        end
                    else
                        allowed = checkAllowed(loadoutKeys)
                    end
...
-- Behandelt Tod, Kill-Cam/Zuschauen und Respawn
...
AddEventHandler('ffa:syncTeams', function(teams)
    ...
    for pid, team in pairs(teams) do
        ...
        if DoesEntityExist(targetPed) then
            if team == myTeam and pid ~= myId then
...
AddEventHandler('ffa:updateHUDStats', function(kills, deaths)
    ...
    if kills > oldKills then SendNUIMessage({ action = 'playSound', sound = 'kill' }) end
