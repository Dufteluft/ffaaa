...
function HandleDeath(killerPed)
    Citizen.CreateThread(function()
        ...
        local remainingRespawn = (currentLobby.respawnTime or 5) - 3
        if remainingRespawn > 0 then
             -- Suche nach einem Ziel zum Zuschauen
            local players = GetActivePlayers()
            local target = -1
            for _, player in ipairs(players) do
                local pPed = GetPlayerPed(player)
                if pPed ~= playerPed and not IsEntityDead(pPed) then
                    target = pPed
                    break
                end
            end
