...
function GiveLoadout(loadoutKeys)
    ...
    -- loadoutKeys can be a string or a table of strings
...
-- Boundaries and Weapon Validation
        ...
        if playerState.isInGame and currentLobby then
            ...
            if map then
                ...
                if currentWeapon ~= GetHashKey('WEAPON_UNARMED') then
                    local allowed = false
                    local lKeys = currentLobby.loadout
                    if type(lKeys) == 'string' then lKeys = { lKeys } end
...
function HandleDeath(killerPed)
    Citizen.CreateThread(function()
        ...
        if currentLobby then
            local waitTime = (currentLobby.respawnTime or 5) - 3
            if waitTime > 0 then
