...
function GiveLoadout(loadoutKeys)
    ...
    -- Falls es ein einzelner String ist, in Table umwandeln
    if type(loadoutKeys) == 'string' then
        loadoutKeys = { loadoutKeys }
    end

    if loadoutKeys and type(loadoutKeys) == 'table' then
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1000)
        if playerState.isInGame and currentLobby and currentLobby.mapId then
            local ped = PlayerPedId()
