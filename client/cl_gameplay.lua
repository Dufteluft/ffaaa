                ...
                if currentWeapon ~= GetHashKey('WEAPON_UNARMED') then
                    ...
                    if type(loadoutKeys) == 'table' then
                        for _, k in ipairs(loadoutKeys) do
                            if checkLoadout(k) then
                                allowed = true
                                break
                            end
                        end
                    else
                        allowed = checkLoadout(loadoutKeys)
                    end
...
function SpawnLobbyVehicle()
    ...
    currentVehicle = CreateVehicle(model, coords.x, coords.y, coords.z, GetEntityHeading(ped), true, false)
    SetEntityAsMissionEntity(currentVehicle, true, true)
    SetModelAsNoLongerNeeded(model)
end

-- Relationship-based Anti-Teamkill
