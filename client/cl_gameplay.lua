...
function GiveLoadout(loadoutKeyOrTable)
    local ped = PlayerPedId()
    RemoveAllPedWeapons(ped, true)

    if type(loadoutKeyOrTable) == 'table' then
        for _, key in ipairs(loadoutKeyOrTable) do
        ...
        local weapons = Config.WeaponLoadouts[loadoutKeyOrTable]
        if weapons then
            for _, weapon in ipairs(weapons) do
                GiveWeaponToPed(ped, GetHashKey(weapon.name), weapon.ammo, false, true)
            end
